...
def simple_simulation(rng: np.random.Generator, sim_length: int):
    ...
    # With a fixed radius, probability is 1/3, with random radius between 0-R,
    # it appears to be around 0.38.
    ...
    ## Angle/position around the cake candles will be placed at
    theta = rng.uniform(0, 2 * np.pi, (sim_length, 5))
    ## Fixed radius from center that candles will be placed at
    radius = R
...
def simulation_animation(
    ...
    # # print([(((x-1) + 2) % 4) + 1 for x in [find_quadrant_rect(x,y) for x,y in cut]])
    ...
    # # for v1,v2 in zip(cut, c1):
    # #     print(cross_prod(v1, v2))
    # # for v1,v2 in zip(cut, c2):
    # #     print(cross_prod(v1, v2))
    # # print(np.sign(np.cross(cut[:,:2], c1)))
